...
const urlSafeBase64Encode = (text: string) =>
  btoa(text).replace(/[+/=]/g, (m) => ({ "+": "-", "/": "_", "=": "" }[m]));

const urlSafeBase64Decode = (text: string) =>
  atob(text.replace(/[-_]/g, (m) => ({ "-": "+", _: "/" }[m])));
...
const is = (
  reqInfo: RequestInfo,
  path: string // if (environment.baseUrl) {
) =>
  //   return false;
  // }

  new RegExp(`${path}(/)?$`, "i").test(reqInfo.req.url);

const getUserFromJWTToken = (req: HttpRequest<any>) => {
